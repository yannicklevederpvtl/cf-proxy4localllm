...
verify: proto
	...
	cd desktop-app && go mod tidy && go test ./...

.PHONY: desktop-app
desktop-app:
	chmod +x desktop-app/scripts/*.sh && ./desktop-app/scripts/build.sh

.PHONY: desktop-release
desktop-release:
	chmod +x desktop-app/scripts/*.sh && ./desktop-app/scripts/package-dmg.sh
